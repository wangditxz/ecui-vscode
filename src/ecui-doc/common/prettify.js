...
(function () {
  ...
  var JAVASCRIPT_EXT_KEYWORDS = 'document window ecui Object Array';
  ...
  var PR_KEYWORD = 'kwd';
  /** token style for a keyword2 */
  var PR_EXT_KEYWORD = 'ekd';
  ...
  function sourceDecorator(options) {
    ...
    if (options['extKeywords']) {
        keywords = options['extKeywords'].replace(/^\s+|\s+$/g, '');
        if (keywords.length) {
          fallthroughStylePatterns.push(
              [PR_EXT_KEYWORD,
               new RegExp('^(?:' + keywords.replace(/\s+/g, '|') + ')\\b'), null]);
  ...
  registerLangHandler(sourceDecorator({
          ...
          'extKeywords': JAVASCRIPT_EXT_KEYWORDS,
          'cStyleComments': true,
          'regexLiterals': true
        }), ['js']);
  registerLangHandler(
